...
def interpolate_greenland_files(CC, source_folder):
    ...
    for file in file_list:
        ...
        interp_file = os.path.join("allgl.nc")
        ...
        ds = nc.Dataset('/Users/tara/Documents/SJSU/MLML/Grids/BedMachine/BedMachineGreenland-v5.nc')
        bm_x = ds.variables['x'][:]
        bm_y = ds.variables['y'][:]
        ds.close()


        # get the extents of bm_x and bm_y
        min_x = np.min(bm_x)
        max_x = np.max(bm_x)
        min_y = np.min(bm_y)
        max_y = np.max(bm_y)
        ...
        CC.extents = [-652925, -3384425, 879625, -632675] # Disko Bay
        ...
        store_interpolated_data(CC, file, interpolated_chla, time,
                                '/Users/tara/Documents/SJSU/MLML/Grids/')
        print("Done! Woohoo!")
